...
func (db *DB) QueryRow(c context.Context, query string, args ...interface{}) *Row {
	return db.connect.queryRow(c, query, args...)
}

// Close closes the connect database, releasing any open resources.
func (db *DB) Close() (err error) {
	if e := db.connect.Close(); e != nil {
...
func (db *DB) Ping(c context.Context) (err error) {
	if err = db.connect.ping(c); err != nil {
		return
	}
	return
}
